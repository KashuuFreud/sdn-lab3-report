...
class ShortestForward(app_manager.OSKenApp):
    ...
    Functions:
    1. Handle ARP broadcast loop.
    2. Forward IPv4 packets according to minimum-delay path.
    3. Install bidirectional flow entries along the path.
    4. Delete old flow entries when port status changes.
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.logger.info(">>> ShortestForward started")
    ...
    def packet_in_handler(self, ev):
        ...
        # Important: ignore LLDP.
        # LLDP is used by OS-Ken topology discovery and should not be handled as normal traffic.
        ...
        if arp_pkt is not None:
            self.handle_arp(msg, in_port, eth_pkt, arp_pkt, pkt)
            return

        if ipv4_pkt is not None:
            self.handle_ipv4(msg, in_port, eth_pkt, ipv4_pkt)
            return

    def handle_arp(self, msg, in_port, eth_pkt, arp_pkt, pkt):
        """
        Handle ARP packets.

        This part combines:
        1. L2 self-learning switch.
        2. ARP broadcast loop suppression used in Task 2.
        """
        datapath = msg.datapath
        ofp = datapath.ofproto
        parser = datapath.ofproto_parser
        ...
        # Only suppress duplicated broadcast ARP Request from different ports.
        ...
        # Install ARP flow only when destination port is known.
    ...
    def handle_ipv4(self, msg, first_in_port, eth_pkt, ipv4_pkt):
        """
        Calculate minimum-delay path and install flow entries.
        """
        src_ip = ipv4_pkt.src
        dst_ip = ipv4_pkt.dst
        ...
        try:
            for i in range(1, len(dpid_path) - 1):
                ...
                in_port = self.network_awareness.link_info[(current_node, previous_node)]
                out_port = self.network_awareness.link_info[(current_node, next_node)]
        ...
        # Install forward and reverse IPv4 flow entries.
        ...
        # Send current packet out from the first switch on the path.
    ...
    def send_flow_mod(self, dpid, src_ip, dst_ip, in_port, out_port):
    ...
    def show_path(self, src_ip, dst_ip, dpid_path, port_path):
        """
        Print selected minimum-delay path and total one-way delay.
        """
        ...
        self.logger.info("delay: %.6f s, estimated RTT: %.6f s",
                         total_delay, total_delay * 2)

    @set_ev_cls(ofp_event.EventOFPPortStatus, MAIN_DISPATCHER)
    def fault_tolerant_handler(self, ev):
        """
        When link status changes, delete old ARP/IPv4 flow entries.

        Then the next packet will trigger PacketIn,
        and the controller will calculate a new minimum-delay path.
        """
        msg = ev.msg
        datapath = msg.datapath
        ofp = datapath.ofproto
        ...
        self.logger.info(">>> port status changed: dpid=%s port=%s reason=%s",
                         datapath.id, msg.desc.port_no, reason)

        for switch in get_switch(self):
            dp = switch.dp
            dp_parser = dp.ofproto_parser

            match_ipv4 = dp_parser.OFPMatch(eth_type=ETH_TYPE_IPV4)
            match_arp = dp_parser.OFPMatch(eth_type=ETH_TYPE_ARP)
        ...
        # Force NetworkAwareness to rebuild topology on next cycle.
        self.network_awareness.topo_map.clear()
        self.network_awareness.link_info.clear()

        self.logger.info(">>> old flows cleared; waiting for topology refresh")
